...
// ProcurementdashboardModuleDeps holds all dependencies for the Procurement Operations composition app.
// ALL use cases are optional — the app degrades gracefully when not wired.
type ProcurementdashboardModuleDeps struct {
	Routes       epkg.Routes
	Labels       epkg.Labels
...
// ProcurementdashboardModule holds all constructed Procurement Operations views.
type ProcurementdashboardModule struct {
	routes           epkg.Routes
	Dashboard        view.View
	Renewals         view.View
	Variance         view.View
...
// NewProcurementdashboardModule constructs the Procurement Operations composition module.
// All view constructors are nil-safe — missing use cases result in empty-state renders.
func NewProcurementdashboardModule(deps *ProcurementdashboardModuleDeps) *ProcurementdashboardModule {
	routes := deps.Routes
	...
	return &ProcurementdashboardModule{
		routes:           routes,
...
func (m *ProcurementdashboardModule) RegisterRoutes(r view.RouteRegistrar) {
