...
import (
	...
	"github.com/erniealice/centymo-golang/domain/procurement/procurementdashboard"
...
type Deps struct {
	Routes       procurementdashboard.Routes
	Labels       procurementdashboard.Labels
...
func renderExpiringTable(rows []ExpiringRow, l procurementdashboard.Labels) template.HTML {
	if len(rows) == 0 {
...
func renderVarianceTable(rows []VarianceRow, l procurementdashboard.Labels) template.HTML {
	if len(rows) == 0 {
		return template.HTML(fmt.Sprintf(
...
func renderRecurrencePanel(count int, l procurementdashboard.Labels) template.HTML {
	if count <= 0 {
		return template.HTML(fmt.Sprintf(
