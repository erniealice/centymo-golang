...
import (
	...
	"github.com/erniealice/centymo-golang/domain/procurement/cost_schedule"
...
// ListViewDeps holds view dependencies for the cost_schedule list.
type ListViewDeps struct {
	Routes                      cost_schedule.Routes
	...
	Labels                      cost_schedule.Labels
	CommonLabels                pyeza.CommonLabels
	TableLabels                 types.TableLabels
}

// PageData holds the data for the cost_schedule list page.
...
// CostScheduleSortSpec is the canonical sort spec.
var CostScheduleSortSpec = espynahttp.SortSpec{
...
var costScheduleSearchFields = []string{"name"}
...
func NewView(deps *ListViewDeps) view.View {
	return view.ViewFunc(func(ctx context.Context, viewCtx *view.ViewContext) view.ViewResult {
		...
		p, err := espynahttp.ParseTableParamsFromSpec(viewCtx.Request, CostScheduleSortSpec)
		if err != nil {
		...
		pageData := &PageData{
			...
			ContentTemplate: "cost-schedule-list-content",
			Table:           tableConfig,
		}
		return view.OK("cost-schedule-list", pageData)
	})
}
...
func buildTableConfig(ctx context.Context, deps *ListViewDeps, status string, p tableparams.TableQueryParams) (*types.TableConfig, error) {
	perms := view.GetUserPermissions(ctx)
	listParams := espynahttp.ToListParams(p, costScheduleSearchFields)
	...
	resp, err := deps.GetCostScheduleListPageData(ctx, &costschedulepb.GetCostScheduleListPageDataRequest{
	...
	if err != nil {
		log.Printf("Failed to list cost schedules: %v", err)
		return nil, fmt.Errorf("failed to load cost schedules: %w", err)
	}

	l := deps.Labels
	columns := costScheduleColumns(l)
	rows := buildTableRows(ctx, resp.GetCostScheduleList(), status, l, deps.Routes, perms)
	...
	tableConfig := &types.TableConfig{
		...
		ShowDensity:          true,
		ShowEntries:          true,
		DefaultSortColumn:    CostScheduleSortSpec.DefaultCol,
		DefaultSortDirection: CostScheduleSortSpec.DefaultDir,
	...
	if status == "active" {
		tableConfig.PrimaryAction = &types.PrimaryAction{
			Label:           l.Buttons.AddCostSchedule,
			ActionURL:       deps.Routes.AddURL,
			Icon:            "icon-plus",
			Disabled:        !perms.Can("cost_schedule", "create"),
...
func costScheduleColumns(l cost_schedule.Labels) []types.TableColumn {
...
func buildTableRows(ctx context.Context, schedules []*costschedulepb.CostSchedule, status string, l cost_schedule.Labels, routes cost_schedule.Routes, perms *types.UserPermissions) []types.TableRow {
	...
	for _, s := range schedules {
		...
		if recordStatus == "active" {
			actions = append(actions, types.TableAction{
				...
				Disabled:        !perms.Can("cost_schedule", "update"),
				DisabledTooltip: l.Errors.PermissionDenied,
			})
			actions = append(actions, types.TableAction{
				Type:            "deactivate",
		...
		actions = append(actions, types.TableAction{
			...
			ItemName:        name,
			Disabled:        !perms.Can("cost_schedule", "delete"),
			DisabledTooltip: l.Errors.PermissionDenied,
		})
...
func statusTitle(l cost_schedule.Labels, status string) string {
	switch status {
	case "active":
