...
type ListViewDeps struct {
	Routes                      procurement.SupplierPlanRoutes
	...
	Labels                      procurement.SupplierPlanLabels
...
func supplierPlanColumns(l procurement.SupplierPlanLabels) []types.TableColumn {
...
func buildTableRows(ctx context.Context, plans []*supplierplanpb.SupplierPlan, status string, l procurement.SupplierPlanLabels, routes procurement.SupplierPlanRoutes, perms *types.UserPermissions) []types.TableRow {
	rows := []types.TableRow{}
	for _, p := range plans {
		active := p.GetActive()
		recordStatus := "active"
...
func statusTitle(l procurement.SupplierPlanLabels, status string) string {
	switch status {
	case "active":
