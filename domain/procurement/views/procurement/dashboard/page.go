...
func renderExpiringTable(rows []ExpiringRow, l procurementdom.ProcurementLabels) template.HTML {
	if len(rows) == 0 {
...
func renderVarianceTable(rows []VarianceRow, l procurementdom.ProcurementLabels) template.HTML {
	if len(rows) == 0 {
		return template.HTML(fmt.Sprintf(
...
func renderRecurrencePanel(count int, l procurementdom.ProcurementLabels) template.HTML {
	if count <= 0 {
		return template.HTML(fmt.Sprintf(
