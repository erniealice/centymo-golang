...
func supplierPlanColumns(l supplier_plan.Labels) []types.TableColumn {
...
func buildTableRows(ctx context.Context, plans []*supplierplanpb.SupplierPlan, status string, l supplier_plan.Labels, routes supplier_plan.Routes, perms *types.UserPermissions) []types.TableRow {
	rows := []types.TableRow{}
	for _, p := range plans {
		active := p.GetActive()
		recordStatus := "active"
...
func statusTitle(l supplier_plan.Labels, status string) string {
	switch status {
	case "active":
