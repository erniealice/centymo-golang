...
func costPlanColumns(l cost_plan.Labels) []types.TableColumn {
...
func buildTableRows(plans []*costplanpb.CostPlan, status string, l cost_plan.Labels, routes cost_plan.Routes, perms *types.UserPermissions) []types.TableRow {
	rows := []types.TableRow{}
...
func statusTitle(l cost_plan.Labels, status string) string {
	switch status {
	case "active":
