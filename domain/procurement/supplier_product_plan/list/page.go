...
func supplierProductPlanColumns(l supplier_product_plan.Labels) []types.TableColumn {
...
func buildTableRows(plans []*supplierproductplanpb.SupplierProductPlan, status string, l supplier_product_plan.Labels, routes supplier_product_plan.Routes, perms *types.UserPermissions) []types.TableRow {
	rows := []types.TableRow{}
...
func statusTitle(l supplier_product_plan.Labels, status string) string {
	switch status {
	case "active":
