...
type ListViewDeps struct {
	Routes          treasury.CollectionRoutes
	...
	RefreshURL      string
	Labels          treasury.CollectionLabels
...
func collectionColumns(l treasury.CollectionLabels) []types.TableColumn {
...
func buildTableRows(collections []*collectionpb.Collection, status string, l treasury.CollectionLabels, routes treasury.CollectionRoutes, perms *types.UserPermissions) []types.TableRow {
	rows := []types.TableRow{}
...
func statusPageTitle(l treasury.CollectionLabels, status string) string {
	switch status {
	case "pending":
...
func statusPageCaption(l treasury.CollectionLabels, status string) string {
	switch status {
	case "pending":
...
func statusEmptyTitle(l treasury.CollectionLabels, status string) string {
	switch status {
	case "pending":
...
func statusEmptyMessage(l treasury.CollectionLabels, status string) string {
	switch status {
	case "pending":
...
func buildBulkActions(l treasury.CollectionLabels, cl pyeza.CommonLabels, status string, routes treasury.CollectionRoutes, perms *types.UserPermissions) []types.BulkAction {
