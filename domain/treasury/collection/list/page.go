...
func collectionColumns(l collection.Labels) []types.TableColumn {
...
func statusPageTitle(l collection.Labels, status string) string {
	switch status {
	case "pending":
...
func statusPageCaption(l collection.Labels, status string) string {
	switch status {
	case "pending":
...
func statusEmptyTitle(l collection.Labels, status string) string {
	switch status {
	case "pending":
...
func statusEmptyMessage(l collection.Labels, status string) string {
	switch status {
	case "pending":
...
func buildBulkActions(l collection.Labels, cl pyeza.CommonLabels, status string, routes collection.Routes, perms *types.UserPermissions) []types.BulkAction {
