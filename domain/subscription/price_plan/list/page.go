...
func pricePlanColumns(l price_plan.Labels) []types.TableColumn {
...
func buildTableRows(pricePlans []*priceplanpb.PricePlan, status string, l price_plan.Labels, cl pyeza.CommonLabels, routes price_plan.Routes, inUseIDs map[string]bool, perms *types.UserPermissions, planNames, scheduleNames map[string]string, durationLabels pyeza.DurationUnitLabels) []types.TableRow {
	rows := []types.TableRow{}
...
func statusPageTitle(l price_plan.Labels, status string) string {
	switch status {
	case "active":
...
func statusPageCaption(l price_plan.Labels, status string) string {
	switch status {
	case "active":
...
func statusEmptyTitle(l price_plan.Labels, status string) string {
	return l.Empty.Title
}

func statusEmptyMessage(l price_plan.Labels, status string) string {
