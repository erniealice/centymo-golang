...
type ListViewDeps struct {
	Routes       subscription.PlanRoutes
	ListPlans    func(ctx context.Context, req *planpb.ListPlansRequest) (*planpb.ListPlansResponse, error)
	GetInUseIDs  func(ctx context.Context, ids []string) (map[string]bool, error)
	Labels       subscription.PlanLabels
...
func planColumns(l subscription.PlanLabels) []types.TableColumn {
...
func buildTableRows(plans []*planpb.Plan, status string, l subscription.PlanLabels, cl pyeza.CommonLabels, routes subscription.PlanRoutes, inUseIDs map[string]bool, perms *types.UserPermissions, clientNames map[string]string, templateNames map[string]string) []types.TableRow {
	rows := []types.TableRow{}
	for _, p := range plans {
		active := p.GetActive()
		recordStatus := "active"
...
func statusTitle(l subscription.PlanLabels, status string) string {
	switch status {
	case "active":
...
func statusSubtitle(l subscription.PlanLabels, status string) string {
	switch status {
	case "active":
