...
type DetailViewDeps struct {
	Routes                 subscription.PricePlanRoutes
	...
	PriceScheduleDetailLabels subscription.PriceScheduleDetailLabels
...
type PageData struct {
	...
	// Routed through the plan-tab handler (subscription.PricePlanEditURL,
...
type ProductPricePlanFormData struct {
	...
	Labels subscription.ProductPricePlanFormLabels
...
func buildPageData(ctx context.Context, deps *DetailViewDeps, id, activeTab string, viewCtx *view.ViewContext) (*PageData, error) {
	...
	editURL := route.ResolveURL(subscription.PricePlanEditURL, "id", pp.GetPlanId(), "ppid", id)
...
func buildTabItems(id string, l subscription.PricePlanLabels, productPriceCount int, routes subscription.PricePlanRoutes) []pyeza.TabItem {
...
func formatBillingKind(kind string, l subscription.PricePlanFormLabels) string {
	switch kind {
...
func formatAmountBasis(basis string, l subscription.PricePlanFormLabels) string {
	switch basis {
...
func basisBannerMessage(amountBasis string, l subscription.PriceScheduleDetailLabels) string {
	switch amountBasis {
	case "AMOUNT_BASIS_DERIVED_FROM_LINES":
...
func loadJobTemplatePhaseOptions(ctx context.Context, deps *DetailViewDeps, parent *priceplanpb.PricePlan, selectedPhaseID string, labels subscription.ProductPricePlanFormLabels) (bool, []types.SelectOption) {
...
func buildBillingModelSummary(
	...
	l subscription.PricePlanDetailLabels2,
...
func pickSummaryCell(kind, basis string, s subscription.PricePlanBillingSummaryCopy) subscription.PricePlanSummaryLines {
	var byBasis subscription.PricePlanSummaryByBasis
	switch kind {
	case "BILLING_KIND_ONE_TIME":
		byBasis = s.OneTime
	case "BILLING_KIND_RECURRING":
	...
	case "BILLING_KIND_AD_HOC":
		byBasis = s.AdHoc
	default:
		return subscription.PricePlanSummaryLines{}
	}
	switch basis {
	case "AMOUNT_BASIS_PER_CYCLE":
