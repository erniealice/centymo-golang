...
func NewMilestoneMarkReadyAction(setStatus SetBillingEventStatusFn, errLabels subscription.ErrorLabels) view.View {
...
func NewMilestoneWaiveAction(setStatus SetBillingEventStatusFn, errLabels subscription.ErrorLabels) view.View {
	return view.ViewFunc(func(ctx context.Context, viewCtx *view.ViewContext) view.ViewResult {
		perms := view.GetUserPermissions(ctx)
		if perms != nil && !perms.Can("milestone", "set_status") {
			return view.HTMXError(errLabels.PermissionDenied)
