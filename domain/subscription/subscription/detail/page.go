...
// hover state.
func resolveRecognizeDisabledTooltip(canRecognize, active bool, l subscription.Labels) string {
	switch {
	case !canRecognize:
...
// pre-resolved customize URL (subscription.CustomizePackageURL with
...
func statusLabelForBillingEventDetail(s billingeventpb.BillingEventStatus, l subscription.MilestoneLabels) string {
	switch s {
	case billingeventpb.BillingEventStatus_BILLING_EVENT_STATUS_READY:
...
func buildInvoicesTable(
	...
	for _, r := range revenues {
		...
		// (sib_revenue_revenue.EmailURL — registered on RevenueRoutes.SendEmailURL).
		// Edit is gated to draft status and uses sib_revenue_revenue.EditURL.
...
func rollupCycleStatus(group []*jobpb.Job, l subscription.OperationsLabels) (string, string, string) {
	hasActive := false
	hasOverdue := false
	allDone := true
	for _, j := range group {
...
func jobTypeLabel(key string, l subscription.JobsTabLabels) string {
	switch key {
	case "shell":
...
func statusLabelForJobStatus(s enums.JobStatus, l subscription.OperationsLabels) string {
	switch s {
	case enums.JobStatus_JOB_STATUS_ACTIVE:
