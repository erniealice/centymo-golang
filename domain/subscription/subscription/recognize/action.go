...
import (
	...
	recognizeform "github.com/erniealice/centymo-golang/domain/subscription/subscription/recognize/form"
...
// buildLabels assembles the recognize drawer's flat Labels from subscription.Labels.
func buildLabels(l subscription.Labels) recognizeform.Labels {
...
func applyResponse(
	...
	if resp != nil {
		...
		if cid := resp.GetConflictingRevenueId(); cid != "" {
			...
			data.ConflictingRevenueURL = strings.ReplaceAll(sib_revenue_revenue.DetailURL, "{id}", cid)
		}
	}
...
func treatmentLabel(t string, labels subscription.Labels) string {
	switch t {
	case "recurring":
...
func statusLabel(s billingeventpb.BillingEventStatus, l subscription.MilestoneLabels) string {
	switch s {
