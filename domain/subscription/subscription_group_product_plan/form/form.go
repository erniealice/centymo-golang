...
// Data is the template data for the subscription_group_product_plan (the
// class) S7 admin add/edit drawer form.
type Data struct {
	FormAction  string
	...
	JobTemplateLabel       string
	JobTemplateOpts        []map[string]any

	// Status — the ACTIVE<->EXCLUDED roster-visibility enum (distinct from
	// Active below, the entity's own soft-delete axis).
	StatusExcluded bool

	Active bool

	Labels       sgpp.FormLabels
	CommonLabels any
}
