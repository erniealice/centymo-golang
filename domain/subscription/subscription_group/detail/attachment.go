...
// attachmentConfig mirrors plan/detail's helper — only EntityType differs.
...
// NewAttachmentUploadAction creates the upload handler for subscription_group attachments.
func NewAttachmentUploadAction(deps *DetailViewDeps) view.View {
	return attachment.NewUploadAction(attachmentConfig(deps))
}

// NewAttachmentDeleteAction creates the delete handler for subscription_group attachments.
