...
var subscriptionGroupSearchFields = []string{"name", "kind"}

func NewView(deps *ListViewDeps) view.View {
	return view.ViewFunc(func(ctx context.Context, viewCtx *view.ViewContext) view.ViewResult {
		perms := view.GetUserPermissions(ctx)
		if !perms.Can("subscription_group", "list") {
			return view.Forbidden("subscription_group:list")
		}
		status := viewCtx.Request.PathValue("status")
		if status == "" {
			status = "active"
		}
		columns := subscriptionGroupColumns(deps.Labels)
		...
		pageData := &PageData{
			...
			ContentTemplate: "subscription-group-list-content",
			Table:           tableConfig,
		}

		return view.OK("subscription-group-list", pageData)
	})
}
...
func buildTableConfig(ctx context.Context, deps *ListViewDeps, status string, columns []types.TableColumn, p tableparams.TableQueryParams) (*types.TableConfig, error) {
	...
	if deps.GetSubscriptionGroupInUseIDs != nil {
		...
		inUseIDs, _ = deps.GetSubscriptionGroupInUseIDs(ctx, itemIDs)
	}
	...
	if status == "active" {
		primaryAction = &types.PrimaryAction{
			...
			Icon:            "icon-plus",
			Disabled:        !perms.Can("subscription_group", "create"),
			DisabledTooltip: fmt.Sprintf(deps.CommonLabels.Errors.MissingPermission, "subscription_group:create"),
		}
	}

	tableConfig := &types.TableConfig{
		ID:                   "subscription-groups-table",
...
func buildTableRows(groups []*subscriptiongrouppb.SubscriptionGroup, status string, l subscription_group.Labels, cl pyeza.CommonLabels, routes subscription_group.Routes, inUseIDs map[string]bool, perms *types.UserPermissions) []types.TableRow {
	rows := []types.TableRow{}
	for _, sg := range groups {
		...
		rows = append(rows, types.TableRow{
			...
			Actions: buildRowActions(id, name, sg.GetActive(), isInUse, l, cl, routes, perms),
		})
	}
	return rows
}

// formatCapacity renders the capacity cell from capacity_mode (+ max_capacity
...
func buildRowActions(id, name string, active, isInUse bool, l subscription_group.Labels, cl pyeza.CommonLabels, routes subscription_group.Routes, perms *types.UserPermissions) []types.TableAction {
	actions := []types.TableAction{
		...
		{Type: "edit", Label: l.Buttons.Edit, Action: "edit", URL: route.ResolveURL(routes.EditURL, "id", id), DrawerTitle: l.Buttons.Edit,
			Disabled: !perms.Can("subscription_group", "update"), DisabledTooltip: fmt.Sprintf(cl.Errors.MissingPermission, "subscription_group:update")},
	}
	...
	if active {
		actions = append(actions, types.TableAction{
			...
			Disabled:        !perms.Can("subscription_group", "create"),
			DisabledTooltip: fmt.Sprintf(cl.Errors.MissingPermission, "subscription_group:create"),
		})
		actions = append(actions, types.TableAction{
			...
			Disabled:        !perms.Can("subscription_group", "update"),
			DisabledTooltip: fmt.Sprintf(cl.Errors.MissingPermission, "subscription_group:update"),
		})
	} else {
		actions = append(actions, types.TableAction{
	...
	} else if !perms.Can("subscription_group", "delete") {
		deleteAction.Disabled = true
		deleteAction.DisabledTooltip = fmt.Sprintf(cl.Errors.MissingPermission, "subscription_group:delete")
	}
...
func statusPageTitle(l subscription_group.Labels, status string) string {
	switch status {
	case "active":
