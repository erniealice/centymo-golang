...
type ListViewDeps struct {
	Routes         product.PriceListRoutes
	...
	Labels         product.PriceListLabels
...
func NewView(deps *ListViewDeps) view.View {
	return view.ViewFunc(func(ctx context.Context, viewCtx *view.ViewContext) view.ViewResult {
		...
		if !perms.Can("price_list", "list") {
			return view.Forbidden("price_list:list")
		}
		_ = perms
...
func buildTableConfig(ctx context.Context, deps *ListViewDeps, columns []types.TableColumn, status string, p tableparams.TableQueryParams) (*types.TableConfig, error) {
	...
	tableConfig := &types.TableConfig{
		...
		PrimaryAction: &types.PrimaryAction{
			...
			DisabledTooltip: fmt.Sprintf(deps.CommonLabels.Errors.MissingPermission, "price_list:create"),
		},
		BulkActions:      &bulkCfg,
		ServerPagination: sp,
	}
...
func priceListColumns(l product.PriceListLabels) []types.TableColumn {
	return []types.TableColumn{
		{Key: "name", Label: l.Columns.Name},
		{Key: "date_start", Label: l.Columns.DateStart, NoSort: true, NoFilter: true, WidthClass: "col-4xl"},
		{Key: "date_end", Label: l.Columns.DateEnd, NoSort: true, NoFilter: true, WidthClass: "col-4xl"},
		{Key: "date_created", Label: "Date Created"},
		{Key: "status", Label: l.Columns.Status, NoFilter: true, WidthClass: "col-2xl"},
	}
}

func buildTableRows(priceLists []*pricelistpb.PriceList, status string, l product.PriceListLabels, cl pyeza.CommonLabels, routes product.PriceListRoutes, inUseIDs map[string]bool, perms *types.UserPermissions) []types.TableRow {
	rows := []types.TableRow{}
	for _, pl := range priceLists {
		...
		if !perms.Can("price_list", "delete") {
			deleteAction.Disabled = true
			deleteAction.DisabledTooltip = fmt.Sprintf(cl.Errors.MissingPermission, "price_list:delete")
		}
		...
		rows = append(rows, types.TableRow{
			...
			Actions: []types.TableAction{
				...
				{Type: "edit", Label: l.Actions.Edit, Action: "edit", URL: route.ResolveURL(routes.EditURL, "id", id), DrawerTitle: l.Actions.Edit, Disabled: !perms.Can("price_list", "update"), DisabledTooltip: fmt.Sprintf(cl.Errors.MissingPermission, "price_list:update")},
				deleteAction,
			},
		})
	}
...
func statusPageTitle(l product.PriceListLabels, status string) string {
	switch status {
	case "active":
...
func statusPageCaption(l product.PriceListLabels, status string) string {
	switch status {
	case "active":
...
func statusEmptyTitle(l product.PriceListLabels, status string) string {
	switch status {
	case "active":
...
func statusEmptyMessage(l product.PriceListLabels, status string) string {
	switch status {
	case "active":
