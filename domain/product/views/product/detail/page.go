...
func buildTabItems(id string, l product.ProductLabels, variantCount, optionCount, lineCount int, routes product.ProductRoutes, showVariantTabs bool) []pyeza.TabItem {
	...
	if deps.ListProductVariants != nil {
		...
		if err != nil {
			...
			for _, v := range varResp.GetData() {
				...
				optionsDisplay := strings.Join(variantOptionLabels[vid], product.OptionValueSeparator)
