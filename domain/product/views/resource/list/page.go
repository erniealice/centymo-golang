...
func NewView(deps *ListViewDeps) view.View {
	return view.ViewFunc(func(ctx context.Context, viewCtx *view.ViewContext) view.ViewResult {
		...
		columns := resourceColumns(deps.Labels)
...
func resourceColumns(l product.ResourceLabels) []types.TableColumn {
	return []types.TableColumn{
		{Key: "name", Label: l.Columns.Name},
		{Key: "description", Label: l.Columns.Description, NoSort: true},
		{Key: "product", Label: l.Columns.Product, NoSort: true},
		{Key: "status", Label: l.Columns.Status, WidthClass: "col-2xl"},
	}
}

func buildTableRows(resources []*resourcepb.Resource, status string, l product.ResourceLabels, routes product.ResourceRoutes, perms *types.UserPermissions) []types.TableRow {
	rows := []types.TableRow{}
...
func statusTitle(l product.ResourceLabels, status string) string {
	switch status {
	case "active":
...
func statusSubtitle(l product.ResourceLabels, status string) string {
	switch status {
	case "active":
