...
func resourceColumns(l resource.Labels) []types.TableColumn {
...
func buildTableRows(resources []*resourcepb.Resource, status string, l resource.Labels, cl pyeza.CommonLabels, routes resource.Routes, perms *types.UserPermissions) []types.TableRow {
	rows := []types.TableRow{}
...
func statusTitle(l resource.Labels, status string) string {
	switch status {
	case "active":
...
func statusSubtitle(l resource.Labels, status string) string {
	switch status {
	case "active":
