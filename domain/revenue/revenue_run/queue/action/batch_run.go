...
// rendered strings.
func buildBatchCompletedTrigger(labels revenuedomain.Labels, result BatchRunResult) string {
	...
	if len(result.RunIDs) == 1 && labels.ViewRunLink != "" {
		runDetailURL := strings.ReplaceAll(revenuedomain.DetailURL, "{id}", result.RunIDs[0])
