...
func attachmentConfig(deps *DetailViewDeps) *attachment.Config {
	return &attachment.Config{
		EntityType:       "revenue_run",
		BucketName:       "attachments",
		RefreshURL:       deps.Routes.DetailTabActionURL,
...
// NewAttachmentUploadAction creates the upload handler for revenue_run attachments.
func NewAttachmentUploadAction(deps *DetailViewDeps) view.View {
	return attachment.NewUploadAction(attachmentConfig(deps))
}

// NewAttachmentDeleteAction creates the delete handler for revenue_run attachments.
