...
// rendered strings.
func buildBatchCompletedTrigger(labels revenuedomain.RevenueRunLabels, result BatchRunResult) string {
	...
	if len(result.RunIDs) == 1 && labels.ViewRunLink != "" {
		runDetailURL := strings.ReplaceAll(revenuedomain.RevenueRunDetailURL, "{id}", result.RunIDs[0])
