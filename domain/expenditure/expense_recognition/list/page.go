	...
	return view.ViewFunc(func(ctx context.Context, viewCtx *view.ViewContext) view.ViewResult {
		...
		pageData := &PageData{
			PageData: types.PageData{
				...
				ActiveSubNav:   "expense-recognitions",
				HeaderTitle:    heading,
...
func recognitionColumns(l expense_recognition.Labels) []types.TableColumn {
...
func buildTableRows(items []*expenserecognitionpb.ExpenseRecognition, l expense_recognition.Labels) []types.TableRow {
	rows := []types.TableRow{}
	for _, r := range items {
		id := r.GetId()
		name := r.GetName()
...
func statusPageTitle(l expense_recognition.Labels, status string) string {
	switch status {
	case "draft":
