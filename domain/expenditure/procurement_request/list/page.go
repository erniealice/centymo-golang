...
func procurementRequestColumns(l procurement_request.Labels) []types.TableColumn {
...
func buildTableRows(requests []*procurementrequestpb.ProcurementRequest, l procurement_request.Labels) []types.TableRow {
	rows := []types.TableRow{}
...
func statusBadgeLabel(l procurement_request.Labels, status string) string {
	switch status {
...
func statusPageTitle(l procurement_request.Labels, status string) string {
	switch status {
	case "PROCUREMENT_REQUEST_STATUS_DRAFT":
