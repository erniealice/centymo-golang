...
func supplierContractColumns(l supplier_contract.Labels) []types.TableColumn {
...
func buildTableRows(contracts []*suppliercontractpb.SupplierContract, l supplier_contract.Labels) []types.TableRow {
	rows := []types.TableRow{}
	for _, c := range contracts {
		id := c.GetId()
		name := c.GetName()
...
func statusPageTitle(l supplier_contract.Labels, status string) string {
	switch status {
	case "SUPPLIER_CONTRACT_STATUS_DRAFT":
