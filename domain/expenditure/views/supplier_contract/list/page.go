...
func supplierContractColumns(l expenditure.SupplierContractLabels) []types.TableColumn {
...
func buildTableRows(contracts []*suppliercontractpb.SupplierContract, l expenditure.SupplierContractLabels) []types.TableRow {
	rows := []types.TableRow{}
	for _, c := range contracts {
		id := c.GetId()
		name := c.GetName()
...
func statusPageTitle(l expenditure.SupplierContractLabels, status string) string {
	switch status {
	case "SUPPLIER_CONTRACT_STATUS_DRAFT":
