...
type DetailViewDeps struct {
	Routes       expenditure.ProcurementRequestRoutes
	Labels       expenditure.ProcurementRequestLabels
...
func NewSubmitAction(deps *DetailViewDeps) view.View {
	return view.ViewFunc(func(ctx context.Context, viewCtx *view.ViewContext) view.ViewResult {
		...
		if !perms.Can("procurement_request", "submit") {
			return view.HTMXError(fmt.Sprintf(deps.CommonLabels.Errors.MissingPermission, "procurement_request:submit"))
		}
...
func NewApproveAction(deps *DetailViewDeps) view.View {
	return view.ViewFunc(func(ctx context.Context, viewCtx *view.ViewContext) view.ViewResult {
		...
		if !perms.Can("procurement_request", "approve") {
			return view.HTMXError(fmt.Sprintf(deps.CommonLabels.Errors.MissingPermission, "procurement_request:approve"))
		}
		if viewCtx.Request.Method != http.MethodPost {
			return view.Error(fmt.Errorf("method not allowed"))
		}
...
func NewRejectAction(deps *DetailViewDeps) view.View {
	return view.ViewFunc(func(ctx context.Context, viewCtx *view.ViewContext) view.ViewResult {
		...
		if !perms.Can("procurement_request", "reject") {
			return view.HTMXError(fmt.Sprintf(deps.CommonLabels.Errors.MissingPermission, "procurement_request:reject"))
		}
		if viewCtx.Request.Method != http.MethodPost {
			return view.Error(fmt.Errorf("method not allowed"))
		}
...
func statusDisplayLabel(l expenditure.ProcurementRequestLabels, status string) string {
	switch status {
	case "PROCUREMENT_REQUEST_STATUS_DRAFT":
...
func strategyDisplayLabel(l expenditure.ProcurementRequestLabels, strategy string) string {
	switch strategy {
...
func fulfillmentModeDisplay(l expenditure.ProcurementRequestLabels, mode string) (label, token string) {
	switch mode {
...
func spawnStatusDisplay(l expenditure.ProcurementRequestLabels, status string) (label, badge string) {
	switch status {
...
func buildLineTable(ctx context.Context, deps *DetailViewDeps, requestID string, l expenditure.ProcurementRequestLabels) *types.TableConfig {
	prIDPtr := requestID
...
func buildSpawnedCellHTML(line *procurementrequestlinepb.ProcurementRequestLine, l expenditure.ProcurementRequestLabels, requestID string, routes expenditure.ProcurementRequestRoutes, poDetailURLTmpl, contractDetailURLTmpl, expenditureDetailURLTmpl string) types.TableCell {
	// Default URL templates when callers have not yet migrated to RouteResult injection.
	if poDetailURLTmpl == "" {
		poDetailURLTmpl = expenditure.PurchaseOrderDetailURL
	}
	if contractDetailURLTmpl == "" {
		contractDetailURLTmpl = expenditure.SupplierContractDetailURL
	}
	if expenditureDetailURLTmpl == "" {
		expenditureDetailURLTmpl = expenditure.ExpenditureExpenseDetailURL
	}
...
func buildSpawnedPOTable(ctx context.Context, deps *DetailViewDeps, requestID string, l expenditure.ProcurementRequestLabels) *types.TableConfig {
