...
func recognitionColumns(l expenditure.ExpenseRecognitionLabels) []types.TableColumn {
...
func buildTableRows(items []*expenserecognitionpb.ExpenseRecognition, l expenditure.ExpenseRecognitionLabels) []types.TableRow {
	rows := []types.TableRow{}
	for _, r := range items {
		id := r.GetId()
		name := r.GetName()
...
func statusPageTitle(l expenditure.ExpenseRecognitionLabels, status string) string {
	switch status {
	case "draft":
