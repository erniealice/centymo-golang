...
func expenseRunColumns(l expense_recognition_run.Labels) []types.TableColumn {
	lc := l.List.Columns
...
func buildTableRows(rows []errshared.ExpenseRecognitionRunRow, l expense_recognition_run.Labels, routes expense_recognition_run.Routes) []types.TableRow {
...
func statusBadge(l expense_recognition_run.Labels, status string) (label, variant string) {
	switch status {
	case "pending":
...
func scopeKindLabel(l expense_recognition_run.Labels, kind string) string {
	switch kind {
	case "supplier":
...
func statusPageTitle(l expense_recognition_run.Labels, status string) string {
	switch status {
	case "pending":
...
func statusEmptyTitle(l expense_recognition_run.Labels, status string) string {
	switch status {
	case "pending":
...
func statusEmptyMessage(l expense_recognition_run.Labels, status string) string {
	switch status {
	case "pending":
