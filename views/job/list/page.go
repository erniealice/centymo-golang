...
func jobColumns(l centymo.JobLabels) []types.TableColumn {
...
func buildTableRows(jobs []*jobpb.Job, status string, l centymo.JobLabels, routes centymo.JobRoutes, perms *types.UserPermissions) []types.TableRow {
	rows := []types.TableRow{}
	for _, j := range jobs {
...
func statusPageTitle(l centymo.JobLabels, status string) string {
	switch status {
	case "draft":
...
func statusPageCaption(l centymo.JobLabels, status string) string {
	switch status {
	case "draft":
