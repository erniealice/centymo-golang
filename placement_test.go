// placement_test.go — the CANONICAL package-cleanup placement gate (v2, Option B).
//
...
var crossCutting = false // centymo is a domain package (product/subscription/revenue/expenditure/procurement/treasury/inventory).
// legacyAllow — each entry carries a dated EXPIRES stamp (no open-ended
// exemptions). Empty = STRICT (the capstone target). centymo's capstone shrank
// this to the GENUINE residuals only — root infra stubs whose service-admin
// import is not yet repointed, the non-Go docs/ planning dir, and the deferred
// services/ checkout surface. Every NAMING drift (pricelist→price_list,
// advancesdashboard→treasuryadvancesdashboard, product/product/line→product/line)
// was RENAMED to canonical, NOT excused.
var legacyAllow = map[string]string{
	// ── R1: root infra stubs still carrying `package centymo` ────────────────
	// service-admin (and external consumer entydad for routes_config.go) still
	// import these root symbols; remove once those imports are repointed at the
	// domain facades / the concern is relocated to its owning package.
	"assets.go":        "AssetsFS //go:embed stub at root (post-Wave-P) — EXPIRES 2026-07-15 (capstone: move asset hosting to pyeza / drop stub)",
	"datasource.go":    "DataSource legacy view-data port at root — EXPIRES 2026-07-15 (capstone: relocate to pyeza, then repoint service-admin import)",
	"labels.go":        "residual root label aliases — EXPIRES 2026-07-15 (capstone: remove once service-admin import is repointed at the domain facades)",
	"routes.go":        "residual root URL consts — EXPIRES 2026-07-15 (capstone: remove once service-admin import is repointed at the domain facades)",
	"routes_config.go": "root compatibility shim — type aliases (Subscription/PriceSchedule/Disbursement/TreasuryAdvances Routes + AdvanceRecognizeMilestone I/O) for external consumer entydad — EXPIRES 2026-07-15 (capstone: remove once entydad re-points at domain/<d> directly)",
	// ── R2: non-Go residue / deferred infra surfaces / charter'd shared leaf ──
	"docs":          "planning markdown, not a Go concern — EXPIRES 2026-07-15 (capstone: relocate under repo docs/ or add docs to the SHARED allowedFirstLevelDirs)",
	"services":      "deferred espyna checkout surface (serial.go/service.go/types.go) — EXPIRES 2026-07-15 (capstone: relocate to espyna, out of scope for the structural waves)",
	"views":         "root views/ dir holding only lf_delegation_parse_test.go after the views/ collapse — EXPIRES 2026-07-15 (capstone: relocate the test under domain/ or tests/ and rmdir)",
	"domain/shared": "charter'd cross-domain leaf (DataSource + Location* helpers) imported by entity view pkgs to break the entity->root->facade cycle — EXPIRES 2026-07-15 (capstone: DataSource->pyeza Wave P, Location*->entydad WL, then drop)",
	// ── R2′: centymo view-only sub-entities with no 1:1 esqyma proto entity ───
	// Each is a genuine centymo view package that maps to NO single esqyma entity
	// (and is NOT one of the three reconciled naming drifts — those were renamed
	// to canonical, not excused). Resolving them requires relocating/splitting
	// logic or a new esqyma entity, both out of scope for a pure structural pass.
...
// (domain/<d>/<subcontext>/<entity>/). EMPTY for centymo — no sub-context layer.
